...
def run_test_generate_points_on_circle():
    ...
    # Test 1:
    expected = [rg.Point(120, 100.0),  # All numbers are approximate.
                rg.Point(100, 120),
                rg.Point(80, 100),
                rg.Point(100, 80)]
...
def run_test_draw_points_on_circle():
    ...
    draw_points_on_circle(window, circle, 40, 'purple')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    title = 'PIZZA tests 4:  8 white slices on purple circle; 20 green slices on blue circle.'
    window = rg.RoseWindow(520, 400, title)
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 10, 'azure', 2.5)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        line = rg.Line(points[k],points[(k + 1) % number_of_segments])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 300, 61, 'lime green', 5)
    window.close_on_mouse_click()
